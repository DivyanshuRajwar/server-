...
router.post('/teacher-signup-form', async function(req, res) {
  ...
  try {
    const { firstName, lastName, username, teacherId, dob, email, password } = req.body;
...
router.post('/submit-student-data', async (req, res) => {
  try {
    ...
    console.log("Received student data:", { name, rollNo, classId, subjectCode, formattedDate });
...
router.get('/get-attendance', async (req, res) => {
  try {
    ...
    if (!date || !classId || !subject) {
      return res.status(400).json({ message: 'All fields are required: date, classId, and subject.' });
    }
